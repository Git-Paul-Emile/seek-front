import { MapPin, Maximize2, BedDouble, ShowerHead, ArrowRight, Heart, BadgeCheck, UtensilsCrossed, Sofa, GitCompareArrows } from "lucide-react";
...
const PropertyCard = ({
  ...
  return (
    <div
      className="group bg-white overflow-hidden hover:shadow-[0_16px_40px_rgba(0,0,0,0.16),0_4px_12px_rgba(0,0,0,0.1)] transition-all duration-300 hover:-translate-y-2 border border-slate-100 flex flex-col h-full w-full"
    ...
    >
              ...
              <CarouselItem key={idx}>
                <div className="relative h-40 sm:h-44 md:h-48 bg-slate-100">
                  <img
                    ...
                    className={`w-full h-full object-cover${bienData?.statutBien?.slug === 'loue' ? ' grayscale' : ''}`}
                    loading="lazy"
                  />
      ...
      <div className="p-3 sm:p-4 flex flex-col flex-1">
        <h3 className="font-semibold text-[#1A2942] text-sm sm:text-base mb-1 line-clamp-1">
          <span className="min-w-0 truncate">{displayProperty.title}</span>
        </h3>

        {/* Badges : statut dispo + vérifié */}
        {isApiData && (bienData?.statutBien || isProprietaireVerified) && (
          <div className="flex flex-wrap items-center gap-1.5 mb-2">
            {bienData?.statutBien && (() => {
              const slug = bienData.statutBien.slug;
              const label = slug === 'libre' ? 'Disponible' : slug === 'loue' ? 'Loué' : bienData.statutBien.nom;
              const cls = slug === 'libre'
            ...
            {isProprietaireVerified && (
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full border bg-blue-50 text-blue-700 border-blue-200 flex items-center gap-1">
        ...
        <div className="flex items-center gap-1.5 text-[11px] text-slate-400 mb-2 line-clamp-1">
        ...
        <div className="flex items-start gap-2 mb-3 sm:mb-4 flex-wrap">
          {[
            ...(displayProperty.surface > 0
              ? [{ icon: Maximize2, tooltip: `${displayProperty.surface} m²`, label: "Surface" }]
              : []),
            ...(displayProperty.bedrooms > 0
              ? [{ icon: BedDouble, tooltip: `${displayProperty.bedrooms} chambre${displayProperty.bedrooms > 1 ? "s" : ""}`, label: `Chambre${displayProperty.bedrooms > 1 ? "s" : ""}` }]
              : []),
            ...(displayProperty.bathrooms > 0
              ? [{ icon: ShowerHead, tooltip: `${displayProperty.bathrooms} salle${displayProperty.bathrooms > 1 ? "s" : ""} de bain`, label: "Sdb" }]
              : []),
            ...((bienData?.nbCuisines ?? 0) > 0
              ? [{ icon: UtensilsCrossed, tooltip: `${bienData!.nbCuisines} cuisine${(bienData!.nbCuisines ?? 0) > 1 ? "s" : ""}`, label: `Cuisine${(bienData!.nbCuisines ?? 0) > 1 ? "s" : ""}` }]
              : []),
            ...((bienData?.nbSalons ?? 0) > 0
              ? [{ icon: Sofa, tooltip: `${bienData!.nbSalons} salon${(bienData!.nbSalons ?? 0) > 1 ? "s" : ""}`, label: `Salon${(bienData!.nbSalons ?? 0) > 1 ? "s" : ""}` }]
              : []),
          ].map(({ icon: Icon, tooltip, label }) => (
            <div key={label} className="relative group/spec flex flex-col items-center gap-0.5">
              ...
              <span className="text-[10px] text-slate-400 leading-none text-center whitespace-nowrap">{label}</span>
        ...
        <div className="flex items-center justify-between pt-2 sm:pt-3 border-t border-slate-50 mt-auto gap-2">
          {/* Bouton Comparer */}
          {isApiData && bienId && (
            <button
              onClick={(e) => {
                e.preventDefault();
                if (bienData) toggleComparison(bienData);
              }}
              disabled={!isSelected(bienId) && isFull}
              title={
                isSelected(bienId)
              ...
              className={`flex items-center gap-1.5 text-xs h-8 px-3 rounded-lg border transition-colors flex-shrink-0 ${
                isSelected(bienId)
                  ? "bg-[#0C1A35] border-[#0C1A35] text-white"
                  : isFull
                  ? "border-slate-200 text-slate-300 cursor-not-allowed"
                  : "border-slate-200 text-slate-500 hover:border-[#0C1A35] hover:text-[#0C1A35]"
              }`}
            >
              <GitCompareArrows className="w-3.5 h-3.5" />
              {isSelected(bienId) ? "Sélectionné" : "Comparer"}
            </button>
          )}
          ...
          {isApiData ? (
            <Link
              ...
              className="bg-[#0C1A35] hover:bg-[#1A2942] text-white text-xs h-8 px-4 rounded-lg transition-colors inline-flex items-center ml-auto"
            >
              Voir détails
            ...
            <Button
              size="sm"
              className="bg-[#0C1A35] hover:bg-[#1A2942] text-white text-xs h-8 px-4 rounded-lg transition-colors ml-auto"
            >
              Voir détails
              <ArrowRight className="w-3.5 h-3.5 ml-1.5" />
            </Button>
