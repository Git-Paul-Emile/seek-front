...
import { Plus, Pencil, Trash2, X, AlertCircle, Loader2, ArrowLeftRight, ToggleLeft, ToggleRight } from "lucide-react";
import { toast } from "sonner";
import {
  useTypeTransactionsAdmin,
  useCreateTypeTransaction,
  useUpdateTypeTransaction,
  useDeleteTypeTransaction,
...
function TypeForm({
  ...
  initial?: TypeTransaction;
  maxOrdre: number;
  onClose: () => void;
  onSuccess: (isEdit: boolean) => void;
}) {
  const create = useCreateTypeTransaction();
  const update = useUpdateTypeTransaction();
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-display text-lg font-bold text-[#0C1A35]">
            {isEdit ? "Modifier le type" : "Nouveau type de transaction"}
          </h2>
...
export default function TypesTransaction() {
  const { data: types = [], isLoading } = useTypeTransactionsAdmin();
  const deleteType  = useDeleteTypeTransaction();
  const toggleActif = useUpdateTypeTransaction();
  ...
  const openEdit   = (t: TypeTransaction) => { setEditing(t); setModalOpen(true); };
  ...
  const handleToggle = (t: TypeTransaction) => {
    setTogglingId(t.id);
    toggleActif.mutate(
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <h1 className="font-display text-2xl font-bold text-[#0C1A35]">Types de transaction</h1>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-slate-400 text-sm">
            Aucun type de transaction. Commencez par en créer un.
          </div>
        ) : (
              ...
              {types.map((type) => {
                ...
                return (
                  <tr key={type.id} className="hover:bg-slate-50/50 transition-colors">
                    {/* Nom */}
                    <td className="px-5 py-3 font-semibold text-[#0C1A35]">{type.nom}</td>
