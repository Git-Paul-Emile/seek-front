...
import {
  ...
  PawPrint, Cigarette, Check, Star, Map as MapIcon, List,
} from "lucide-react";
...
const RecherchePage = () => {
  ...
  // Vue liste / carte (mode proximité)
              ...
              return (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-bold text-white tabular-nums">{r} km</span>
                  </div>
      ...
      <div className="lg:flex">
              ...
              return (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <span className="text-xs font-semibold" style={{ color: zone.color }}>{zone.label}</span>
                      <span className="text-slate-400 text-xs ml-1.5">· {zone.sub}</span>
                    </div>
                    <span className="text-sm font-bold text-slate-700 tabular-nums">{r} km</span>
                  </div>
                  <div className="relative h-6 flex items-center">
                  ...
                  {!isLoading && total === 0 && (
                    <div className="mt-3 p-2.5 bg-amber-50 border border-amber-200 rounded-lg">
                      <p className="text-xs text-amber-700 font-medium mb-1">Aucun bien dans ce rayon</p>
                      <p className="text-xs text-amber-600">Déplacez le curseur vers la droite pour élargir la zone.</p>
                    </div>
                  )}
                </div>
              );
        ...
        <div className="flex items-start justify-between mb-5 flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {/* Toggle liste / carte — visible uniquement en mode proximité */}
            {isProximityMode && !isLoading && displayItems.length > 0 && (
              <div className="flex items-center rounded-xl border border-slate-200 overflow-hidden bg-white shadow-sm">
                <button
                  type="button"
                  onClick={() => setMapView(false)}
                  className={`flex items-center gap-1.5 px-3 py-2 text-xs font-medium transition-colors ${!mapView ? "bg-[#0C1A35] text-white" : "text-slate-500 hover:bg-slate-50"}`}
                >
        ...
        {/* ── Carte en mode proximité ── */}
        {isProximityMode && mapView && !isLoading && displayItems.length > 0 && (
          <div className="mb-6 px-4 sm:px-0">
            <CarteAnnonces
              items={displayItems}
              proximityPoint={isProximityMode && proximityLat && proximityLng
                ? { lat: proximityLat, lng: proximityLng, label: proximityLabel }
                : null}
            />
          </div>
        )}
